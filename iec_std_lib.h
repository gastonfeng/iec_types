...
#define __DATE_LITERAL(value) __literal(IEC_DATE,value,)
...
#define __INIT_IEC_DATE (IEC_DATE){0,0}
...
#define __TIME_CMPP(t1, t2) (t2->tv_sec == t1.tv_sec ? t1.tv_nsec - t2->tv_nsec : t1.tv_sec - t2->tv_sec)
...
static inline TIME __date_and_time_to_date(EN_ENO_PARAMS, TIME IN)
{
    ...
    return (TIME)
    {
        IN.tv_sec - (IN.tv_sec % (24 * 60 * 60)), 0
    };
}
...
static inline TIME __string_to_time(EN_ENO_PARAMS, STRING IN)
{
    ...
    if(l != 0)
    {
        ...
        return (TIME)
        {
            IN_val, (IN_val - (LINT)IN_val) * 1000000000
        };
    }
    else
    {
...
static inline STRING __date_to_string(EN_ENO_PARAMS, IEC_DATE IN)
{
    STRING res;
    struct tm *broken_down_time;
    time_t seconds;
...
#define VA_ARGS_IEC_DATE IEC_DATE
